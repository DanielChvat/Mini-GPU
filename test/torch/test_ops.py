...
except ImportError as exc:  # pragma: no cover - used when torch is not installed.
...
class MiniGpuTorchOpsTest(unittest.TestCase):
    ...
    def test_add_mul_div(self):
        a = torch.tensor([1.0, -2.0, 3.5, 8.0], dtype=torch.float32)
        b = torch.tensor([2.0, 4.0, -0.5, 0.25], dtype=torch.float32)
    ...
    def test_sqrt_and_reciprocal(self):
        x = torch.tensor([0.25, 1.0, 2.0, 9.0], dtype=torch.float32)
        gx = self.to_minigpu(x)
    ...
    def test_exp_sigmoid_tanh(self):
        ...
        self.assert_tensor_close(torch.exp(gx), torch.exp(x), rtol=2e-1, atol=2e-1)
        self.assert_tensor_close(torch.sigmoid(gx), torch.sigmoid(x), rtol=2e-1, atol=2e-1)
        self.assert_tensor_close(torch.tanh(gx), torch.tanh(x), rtol=2e-1, atol=2e-1)

    def test_log_log2_pow(self):
        x = torch.tensor([0.5, 1.0, 2.0, 4.0], dtype=torch.float32)
        y = torch.tensor([1.0, 2.0, 0.5, 3.0], dtype=torch.float32)
        gx = self.to_minigpu(x)
        gy = self.to_minigpu(y)

        self.assert_tensor_close(torch.log(gx), torch.log(x), rtol=1e-4, atol=1e-4)
        self.assert_tensor_close(torch.log2(gx), torch.log2(x), rtol=1e-4, atol=1e-4)
        self.assert_tensor_close(torch.pow(gx, gy), torch.pow(x, y), rtol=1e-4, atol=1e-4)
        self.assert_tensor_close(torch.pow(gx, 2.0), torch.pow(x, 2.0), rtol=1e-4, atol=1e-4)

    def test_log10_and_trig(self):
        x = torch.tensor([0.5, 1.0, 2.0, 4.0], dtype=torch.float32)
        angles = torch.tensor([-1.0, -0.5, 0.0, 0.75, 1.0], dtype=torch.float32)
        gx = self.to_minigpu(x)
