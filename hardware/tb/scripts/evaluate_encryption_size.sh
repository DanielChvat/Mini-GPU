...
yosys -Q -p "read_verilog \
  ...
  hardware/rtl/security/encryption/aes/*.v \
  hardware/rtl/security/encryption/chacha/*.v \
  hardware/rtl/security/encryption/rosc/*.v \
  hardware/rtl/security/encryption/*.v \
  hardware/rtl/common/*.v
synth_xilinx -family xc7 -top encryption_security_controller; stat"
