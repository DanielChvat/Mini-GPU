...
set num_cores 1
set num_warps_per_core 1
set shared_float_units 1

if {[info exists ::env(MGPU_NUM_CORES)]} {
    set num_cores $::env(MGPU_NUM_CORES)
}

if {[info exists ::env(MGPU_NUM_WARPS_PER_CORE)]} {
    set num_warps_per_core $::env(MGPU_NUM_WARPS_PER_CORE)
}

if {[info exists ::env(MGPU_SHARED_FLOAT_UNITS)]} {
    set shared_float_units $::env(MGPU_SHARED_FLOAT_UNITS)
}

set warp_id_width 1
if {$num_warps_per_core > 2} {
    set warp_id_width 2
}
...
puts "Mini-GPU Basys3 variant: NUM_CORES=$num_cores NUM_WARPS_PER_CORE=$num_warps_per_core SHARED_FLOAT_UNITS=$shared_float_units"
synth_design -top basys3_mini_gpu_top -part xc7a35tcpg236-1 \
    -generic "NUM_CORES=$num_cores" \
    -generic "NUM_WARPS_PER_CORE=$num_warps_per_core" \
    -generic "WARP_ID_WIDTH=$warp_id_width" \
    -generic "SHARED_FLOAT_UNITS=$shared_float_units"
report_utilization
